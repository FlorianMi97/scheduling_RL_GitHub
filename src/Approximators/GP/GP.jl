...
"""
    ...
    (optional) kwargs:  stationary_samples::Bool = true # if false change samples each x generations
                        ...
                        .....
"""
function createGP(envs; objective = [], kwargs...)
...
function drawTree(tree_expr::Expr, stage=1, reduced = false, feature = nothing)
    ...
    if reduced == false
        savefig(plot!(), string(string(@__DIR__),"/Plots/treePlot$stage.svg"))
    else
        savefig(plot!(), string(string(@__DIR__),"/Plots/treePlot$(stage)reducedBy$(feature).svg"))
    end
end
